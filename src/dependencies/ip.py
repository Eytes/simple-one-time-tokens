...
def get_requesters_ip(request: Request) -> str:
    """
    Извлекает IP-адрес клиента из объекта запроса.

    **Параметры:**
    - `request` (Request): Объект запроса FastAPI, содержащий информацию о клиенте.

    **Возвращаемое значение:**
    - `str`: IP-адрес клиента.

    **Описание:**
    Функция получает IP-адрес клиента, отправившего запрос.
    Используется для авторизации, логирования и проверки безопасности.
    """
